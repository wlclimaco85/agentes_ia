...
$envChecks = @{
    "JAVA_HOME" = if ($env:JAVA_HOME) { $env:JAVA_HOME } else { "C:\dharatecnologia\tools\jdk8\jdk8u492-b09" }
    "GRADLE_USER_HOME" = if ($env:GRADLE_USER_HOME) { $env:GRADLE_USER_HOME } else { "C:\dharatecnologia\tools\gradle-cache" }
